...
contract WhiteLightAggregator is AccessControl, IWhiteLightAggregator {
    ...
    bytes public versionBytes; // chain id of the network where the confirmations are collected
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < _trxData.length; i++) {
            ...
            bytes memory unsignedTrx =
                getUnsignedTrx(_trxData[i][0], _submissionId);
    ...
    /// @param _submissionId Submission identifier.
    function getUnsignedTrx(bytes memory _payloadPart, bytes32 _submissionId)
        public
        view
