...
contract MockDeBridgeGate is DeBridgeGate {
    ...
    function mock_set_deBridgeInfo(
        ...
        info.lookedInStrategies=lookedInStragegis; // total locked assets in strategy (AAVE, Compound, etc)
    ...
    function mock_set_chainSupportInfo(uint256 chainId, bool isSupported, uint256 fixedNativeFee, uint256 transferFeeBps) public {
        ChainSupportInfo memory info = ChainSupportInfo(isSupported, fixedNativeFee, transferFeeBps);
    ...
    function mock_set_deFiController(address mock) public {
         defiController=IDefiController(mock);
    ...
    function mock_set_excessConfirmations(uint256 amount) public {
    ...
    function call_internal_claim(
        ...
        _claim(_submissionId, _debridgeId, _receiver, _amount, _fallbackAddress, _executionFee, _data);
    }
    ...
    function call_internal_send(
       ...
       bool useAssetFee
        ...
        _send(tokenAddress, debridgeId, amount, chainIdTo, useAssetFee);
    }
    ...
    function call_internal_add_asset(bytes32 debridgeId, address token, uint256 chainId) public {
        _addAsset(debridgeId, token,  chainId);
    }

    function call_internal_checkConfirmations(bytes32 _submissionId, bytes32 _debridgeId, uint256 _amount, bytes[] memory _signatures) public {
