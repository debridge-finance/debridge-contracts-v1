...
contract("DeBridgeGate light mode", function () {
  ...
  context("Test managing assets", () => {
    ...
    it("should add external asset if called by the admin", async function () {
      ...
      for (let i = 0; i < oracleKeys.length; i++) {
        ...
        signatures.push((await bscWeb3.eth.accounts.sign(deployId, oracleKey)).signature);
      }
  ...
  context("Test mint method", () => {
    ...
    before(async function () {
      ...
      for (let i = 0; i < oracleKeys.length; i++) {
        const oracleKey = oracleKeys[i];
        this.signatures.push((await bscWeb3.eth.accounts.sign(submission, oracleKey)).signature);
      }
    });
  ...
  context("Test burn method", () => {
    ...
    it("should reject burning from current chain", async function () {
      ...
      const signature = "0x";
      await expectRevert(
        this.debridge.burn(debridgeId, receiver, amount, 42, deadline, signature, {
          from: alice,
        }),
        "burn: native asset"
      );
    ...
    before(async function () {
      ...
      for (let i = 0; i < oracleKeys.length; i++) {
        ...
        this.ethSignatures.push(
          (await bscWeb3.eth.accounts.sign(curentChainSubmission, oracleKey)).signature
        );
      }
      ...
      for (let i = 0; i < oracleKeys.length; i++) {
        ...
        this.erc20Signatures.push(
          (await bscWeb3.eth.accounts.sign(erc20Submission, oracleKey)).signature
        );
      }
    });
